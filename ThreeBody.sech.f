cSType234567890
      ...
      double precision, allocatable :: lPsi(:,:),mPsi(:,:),rPsi(:,:),PlotPsi(:,:),Energies(:,:)
...
c read in boundary conditions
      read(5,*)
      read(5,*)
      read(5,*) Shift,Shift2,Order, Left, Right, Top, Bottom
...
c read in the hyperangular domain
      read(5,*)
      read(5,*)
...
c      xMin = 0.0d0              ! units of Pi
c      xMax = 1.0d0              ! units of Pi
c      Left = NLeft              ! phi (x) boundary condition at 0
c      Right = NRight            ! phi (x) boundary condition at pi

c      yMin = 0.0d0     ! units of Pi
c      yMax = 0.5d0     ! units of Pi
c      Bottom = 2       ! theta (y) boundary condition at 0 -> no BC imposed (function is finite)
c      Top = 2          ! theta (y) boundary condition at pi/2 -> no BC imposed (function is finite)
 ...
 55   continue
      ...
      write(6,*) 'mu =',mu
      write(6,*) 'DcoefAA = ',DcoefAA,' DcoefAB = ',DcoefAB
...
c      open(100,File='fort.100',STATUS='NEW',BLOCKSIZE=512)
c      open(99, File='fort.99', STATUS='NEW',BLOCKSIZE=512)
c      open(98, File='fort.98', STATUS='NEW',BLOCKSIZE=512)
      write(100,*)
      do iR = 1,RSteps
         call cpu_time(time1)
         Rvalue = R(iR)
...
c       endif
c       !write(6,*) R(iR)
         write(6,*) 'Shift ', Shift, '   Shift2', Shift2
...
c     call GridMaker(R(iR),xNumPoints,xMin,xMax,yNumPoints,yMin,yMax,xPoints,yPoints,d31,RscaleAB,RscaleAA,r0xfactor,r0yfactor)
         ...
         call GridMakerBosons(R(iR),xNumPoints,xMin,xMax,yNumPoints,yMin,yMax,xPoints,yPoints,d12,d31,d23,
     .        phi12,phi31,phi23,RscaleAA,r0xfactor,r0yfactor)
...
c     mu = 0.5d0
     ...
     >        MatrixDim,HalfBandWidth,H)
         ...
         write(6,*) R(iR)
         do k = 1,NumStates
            write(6,21) k,Energies(k,1),Energies(k,2)
         enddo
         open(100,POSITION="APPEND")
         write(100,20) R(iR),(Energies(k,1),k=1,NumStates)
         close(100)
...
c     write(*,11)R(iR),(Energies(k,1),k=1,NumStates),Shift,Shift2,timep-secp
         call cpu_time(timep)
         secp = timep 
...
c     write(9999+iR,20) (mPsi(k,Index(l)), l = 1,NumPrintStates)
         ...
         call cpu_time(time2)
         write(6,*) 'time for one step = ',time2-time1
      enddo
   ...
   10 format(f12.8,1P,100e25.15)
   11 format(f16.12,1P,100e20.12)
  111 format(f16.12,1P,100I5)
   20 format(1P,100e20.12)
 21   format(i4,100e20.12)
...
c     %%%%%%%%%%  The following uses the parameterization of B. R. Johnson J. Chem Phys 73 5051
...
c     Smooth Grid (jpdincao 10-06-03)
      do k=1,xNumPoints/5
          ...
          enddo
      enddo
      write(777,*) '# xNumPoints= ',xNumPoints
      do k=1,xNumPoints
...
c     Smooth Grid (jpdincao 10-06-03)
      ...
      subroutine GridMakerBosons(R,xNumPoints,xMin,xMax,yNumPoints,yMin,yMax,xPoints,yPoints,d12,d31,d23,
     .     phi12,phi31,phi23,RscaleAA,r0xfactor,r0yfactor) 

      integer xNumPoints,yNumPoints,ncount
      double precision R,xMin,xMax,yMin,yMax,RscaleAB,RscaleAA,r0xfactor,r0yfactor
      double precision d12,d31,d23,x0,xDeltBig,DeltTest
      double precision phi12,phi31,phi23
      double precision xPoints(xNumPoints),yPoints(yNumPoints)
      ...
      phi31c = MOD(Pi-phi31,2.d0*Pi)
c      phi23c = Pi

      r0xAA = r0xfactor*RscaleAA
c      r0xAB = r0xfactor*RscaleAB
 
      ax = (dsqrt(2.d0)*r0xAA/(d31*R))**2-1.d0
c      axc = dsin(phi31c/2.d0-Pi/2.d0) !-dsqrt(3.d0)/2.d0
      axc = 0.5d0!-dsqrt(3.d0)/2.d0
      if (ax.ge.axc) ax = axc
      xC1 = -dasin(ax)/Pi-1.d0/2.d0+phi31c/Pi
      xC2 =  dasin(ax)/Pi+1.d0/2.d0+phi31c/Pi
c      ax = (dsqrt(2.d0)*r0xAA/(d23*R))**2-1.d0
c      axc = -dsqrt(3.d0)/2.d0
c      if (ax.ge.axc) ax = axc
c      xC3 = -dasin(ax)/Pi-1.d0/2.d0+phi23c/Pi
      xNumPoints1 = xNumPoints/8
      xNumPoints2 = 3*xNumPoints/8
      xNumPoints3 = xNumPoints-xNumPoints1-xNumPoints2
      xDelt1 = (xC1-xMin)/(xNumPoints1-1)
      do i1 = 1,xNumPoints1
         xPoints(i1) = ((i1-1)*xDelt1+xMin)*Pi
      enddo
...
cc         TempShift = TempShift - (TempEnergies(TempNumStates,1) - TempEnergies(TempNumStates-1,1))
c        endif
c        !write(6,*) TempShift
